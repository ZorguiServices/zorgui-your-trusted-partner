...
const BookingSection = () => {
  ...
  return (
    <section id="booking" className="py-20 bg-muted" ref={ref}>
      <div className="container mx-auto px-4 max-w-2xl">
        <h2 className={`text-3xl md:text-4xl font-bold text-center text-primary mb-4 scroll-reveal ${inView ? 'revealed' : ''}`}>{t.booking.title}</h2>
        ...
        <form onSubmit={handleSubmit} className={`bg-card rounded-2xl p-6 md:p-8 shadow-card space-y-4 hover-lift scroll-reveal-scale ${inView ? 'revealed' : ''}`} style={{ transitionDelay: '200ms' }}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="relative">
              ...
              <input type="date" required value={form.date} onChange={e => setForm({ ...form, date: e.target.value })}
                className="w-full ps-10 pe-4 py-3 rounded-lg border border-border bg-background focus:ring-2 focus:ring-gold outline-none transition hover:border-gold/50" />
            </div>
            <div className="relative">
          ...
          <select value={form.service} required onChange={e => setForm({ ...form, service: e.target.value })}
            className="w-full px-4 py-3 rounded-lg border border-border bg-background focus:ring-2 focus:ring-gold outline-none transition hover:border-gold/50">
          ...
          <button type="submit" disabled={loading} className="w-full py-4 bg-primary text-primary-foreground font-bold text-lg rounded-lg hover:bg-navy-light transition-all flex items-center justify-center gap-2 hover:scale-[1.02] disabled:opacity-60 active:scale-95">
