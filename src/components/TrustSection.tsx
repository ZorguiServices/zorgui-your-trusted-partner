  ...
  return (
    <section className="py-16 bg-muted" ref={ref}>
      <div className="container mx-auto px-4">
        <h2 className={`text-3xl font-bold text-center text-primary mb-10 scroll-reveal ${inView ? 'revealed' : ''}`}>{t.trust.title}</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {items.map((item, i) => (
            <div
              key={i}
              className={`flex items-center gap-4 bg-card rounded-xl p-6 shadow-card hover-glow cursor-default ${inView ? 'animate-fade-in-up' : 'opacity-0'}`}
